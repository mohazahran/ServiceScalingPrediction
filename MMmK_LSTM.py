...
class MMmK_LSTM(nn.Module):
    def __init__(self, inputWin = 1, lstmHiddenDim = 1, lr = 0.01, gradientClip = 10.0, gradientClipType = 'inf', optim = 'SGD'):
        super(MMmK_LSTM, self).__init__()
        ...
        self.lstm = nn.LSTM(input_size=1,
                           hidden_size=lstmHiddenDim,
                           num_layers=1,
                           bias=True)
        self.linear = nn.Linear(self.lstmHiddenDim, 1)
        self.relu = nn.ReLU()
    ...
    def MMmKPass(self, inp, clampNumbers = False):
    ...
    def forwardPass(self, inp, clampNumbers = False):
        logPK = self.MMmKPass(inp, clampNumbers)
        PK = torch.exp(logPK)
        PK = PK.view(inp.size(0), 1, 1)
        output, (h_n, c_n) = self.lstm(PK)
    ...
    def predict(self, inp, clampNumbers = True):
        return self.forwardPass(inp)
    
    def format_trainingData_window(self, data_X, data_Y):
        newData_X = []
        newData_Y = []
        timeStep = 0
        startSpoint = 0
        for i in range(len(data_X)):
            if data_X[i] == -1:
                ...
                continue
            inp = []
            if timeStep < self.inputWin:
                inp = [0.0]*(self.inputWin - timeStep - 1)
                inp += data_X[startSpoint : i+1]
            else:
                inp = data_X[i-self.inputWin+1 : i+1]
...
def evaluate(MMmK, data_X, data_Y):
    MMmK.eval()
    totalLoss = 0
    batches_X, batches_Y = MMmK.batchfiy(data_X, data_Y)
...
def train(MMmK, train_X, train_Y, valid_X, valid_Y, test_X, test_Y, epochs, batchsize, annelingAt = 20, shuffleData = False, showBatches = False):
    ...
    batches_X, batches_Y = MMmK.batchfiy(train_X,train_Y)
    ...
    for e in range(epochs):
        ...
        if (validLoss < bestValidLoss) or (validLoss == bestValidLoss and trainLoss < train_bestValidLoss) :
            ...
            torch.save(bestModel, 'MMmK_LSTM'+MMmK.toStr())
    ...
    #torch.save(bestModel, 'MMmK_LSTM_'+bestModel.toStr())
...
def main():
    ...
    #(self, inputWin = 1, lstmHiddenDim = 1, lr, gradientClip = 10.0, gradientClipType = 'inf', optim = 'SGD'):
    MMmK = MMmK_LSTM(inputWin=inputWin, lstmHiddenDim=lstmHiddenDim  ,lr=0.1, gradientClip = 5.0, gradientClipType = 2, optim = 'Adam')
    
    data_X, data_Y = getTrainingData(dir, summaryFile, minDropRate=100, maxDropRate=1e10)
    ...
    train(MMmK, 
          ...
          valid_X, valid_Y, 
          test_X, test_Y,
          epochs = 10000, batchsize = 32, annelingAt = 5,
          shuffleData = False,
          showBatches = True
          )
