...
class useGenericQueue_learn_MMmK_multipleMus(nn.Module):
    def __init__(self, initialMu = 10, K=10, lossType = 'MSE', power_P = 5, eps = 0.01,
        ...
        super(useGenericQueue_learn_MMmK_multipleMus, self).__init__()
...
def main():
    ...
    model = useGenericQueue_learn_MMmK_multipleMus( 
        initialMu = 25.0, 
        K=true_K, 
        lossType = 'NLL', #'MSE', 'NLL'
        power_P = min(true_K, 10),
        eps = 0.00001,
        lr=0.01, gradientClip=0.5, gradientClipType=2, optim='Adam'
        )
