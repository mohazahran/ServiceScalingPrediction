...
class GenericQueue_customGradients(nn.Module):
    def __init__(self, params):
        ...
        if params['modelType'] == 'MMmK':
            ...
            for i in range(self.params['K'] - 1):
                ...
                #self.mus[str(0)] = nn.Parameter(torch.DoubleTensor([97.821807]), requires_grad=True)
                #self.mus[str(1)] = nn.Parameter(torch.DoubleTensor([115.851269]), requires_grad=True)
                #self.mus[str(2)] = nn.Parameter(torch.DoubleTensor([135.157496]), requires_grad=True)
                #self.mus[str(3)] = nn.Parameter(torch.DoubleTensor([156.583410]), requires_grad=True)
            

        elif params['modelType'] == 'embeddedMC': # each state has a different mu for all previous states
            cntr = 0
            for i in range(0, self.params['K']-1):
                for j in range(-1,0):
                    self.mus[str(cntr)] = nn.Parameter(torch.FloatTensor([params['initialMu']]), requires_grad=True)
                    cntr += 1
    ...
    def mus_2_str(self):
        ...
        for i in range(self.paramCount):
            str_mus += 'mu%d=%f '%(i,self.mus[str(i)].item())
        return str_mus
            ...
            for j in range(self.K):
                try:
                    ...
                    for k in range(self.paramCount):
                        dP_dmus[k][i][j] = self.mus[str(k)].grad
        ...
        for b in range(len(inputLambdas)):
            ...
            if (b) % batchSize == 0:
                ...
                for p in range(model.paramCount):
                    model.mus[str(p)].grad = torch.DoubleTensor([gradient[p]])
                ...
                if showBatches:
                    print 'batch#', b, 'loss=', total_loss.item(), model.mus_2_str(), 'grad=', gradient
                gradient = 0.0
                total_loss = 0.0
...
def run_using_MMmK_simulation():
    ...
    params = {
        ...
        'modelName': 'genericQueueModel_muPerState_K5_simulatedData_customGradient',
        'optim': 'Adam',
        'lr': 0.1,
...
def run_using_real_data():
    ...
    data_X, data_Y = getTrainingData(dir, summaryFile, minDropRate=1, maxDropRate=10, useDropRate = True)
    ...
    params = {
        ...
        'modelName': 'genericQueueModel_muPerStates_K5_realData_customGradient_5_100',
        'optim': 'Adam',
        'lr': 0.1,
        'use_lr_scheduler': False,
        'gradientClip': 10.0  # for exploding gradients
    }
